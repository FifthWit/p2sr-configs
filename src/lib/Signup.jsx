...
export default function Signup() {
    ...
    return (
        <>
        {(!pb.authStore.model || !pb.authStore.model.username || pb.authStore.model.username === '') ? (        
        <div className="flex flex-col bg-primary dark:bg-dark-primary h-auto w-64 rounded-lg m-4">
            ...
            <h1 className="font-bold text-xl">Logged in as {pb.authStore && pb.authStore.model && pb.authStore.model.username}!</h1>
            <p>(sorry but you have to reload the page if you want to upload configs, will hopefully fix later)</p>
        </div>
        )}
        </>
    )
